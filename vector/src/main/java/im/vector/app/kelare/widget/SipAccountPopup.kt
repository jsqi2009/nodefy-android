...
class SipAccountPopup(context: Context, private val mBus: AndroidBus, accountList: ArrayList<DialerAccountInfo>, private val isCall:Boolean, private val isVM:Boolean) : RelativePopupWindow(context),
    ...
    init {
        ...
        contentView = LayoutInflater.from(context).inflate(R.layout.popup_sip_acount_list, null)
    ...
    private fun circularReveal(anchor: View) {
        (contentView as CircularRevealCardView).run {
            post {
                ...
                measure(
                        makeDropDownMeasureSpec(this@SipAccountPopup.width, windowRect.width()),
                        makeDropDownMeasureSpec(this@SipAccountPopup.height, windowRect.height())
                )
