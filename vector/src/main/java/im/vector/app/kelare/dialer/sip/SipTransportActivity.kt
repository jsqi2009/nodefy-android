...
class SipTransportActivity : VectorBaseActivity<ActivitySipTransportBinding>(), View.OnClickListener, RecyclerItemClickListener {
    ...
    private fun renderData() {
        portList.clear()
        portList.add(SipTransportInfo("UDP", false))
        portList.add(SipTransportInfo("TCP", false))
        portList.add(SipTransportInfo("TLS", false))
    ...
    override fun onRecyclerViewItemClick(view: View, position: Int) {
        ...
        mBus.post(SelectValueEvent(1, portList[position]!!.name!!))
        finish()
    }
