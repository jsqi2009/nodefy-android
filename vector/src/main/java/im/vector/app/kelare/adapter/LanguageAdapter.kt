...
class LanguageAdapter(private val mContext: Activity, private val  selectedType: String, private val mOnItemClickListener: RecyclerItemClickListener) : AbstractBaseRecyclerAdapter<Locale>(mContext) {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {

        var context = LayoutInflater.from(mContext)
        var view = context.inflate(R.layout.item_language_list, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        holder.tvType.text = info.getDisplayLanguage(info)
        ...
        if (getDataList()!!.size == 1) {
            ...
            if (selectedType == info.getDisplayLanguage(info)) {
                if (position == 0) {
