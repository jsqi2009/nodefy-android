 ...
 *  desc   :
 */
class SipChatRoomAdapter(private val mContext: Activity, private val mOnItemClickListener: RecyclerItemClickListener, private val mLongClickListener: OnItemLongClickListener) : AbstractBaseRecyclerAdapter<SipChatRoom>(mContext) {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        var context = LayoutInflater.from(mContext)
        var view = context.inflate(R.layout.item_sip_chat_room_list, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, @SuppressLint("RecyclerView") position: Int) {
        ...
        holder.tv_username.text = info.peerUserName + " (" + info.localUserName + ")"
        holder.tv_message.text = info.lastMessage
        holder.iv_logo.setImageDrawable(
                AvatarGenerator.AvatarBuilder(context)
                        .setLabel(info.peerUserName!!)
                        .setAvatarSize(120)
                        .setTextSize(30)
                        .toSquare()
                        .toCircle()
